// app/api/socios/gasto-compartido/route.ts
// Gasto compartido entre socios — descuenta de ahorros individuales, no requiere ronda activa
...
export async function POST(req: Request) {
  try {
    ...
    // Reparto equitativo: base redondeado hacia abajo, el último absorbe el residuo
    ...
    const insuficientes = montosPorSocio.filter(({ socio, monto: m }) => Number(socio.saldoAhorros) < m);
    ...
    await prisma.$transaction(async (tx) => {
      for (const { socio, monto: m } of montosPorSocio) {
        ...
        await tx.movimientoCuenta.create({
          data: {
            socioId: socio.id,
            rondaId: null,   // no requiere ronda activa
            tipo: "RETIRO",
            monto: montoDecimal,
            nota,
          },
