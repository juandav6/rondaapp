  ...
  try {
    ...
    const [acums, ahorrosSemana, aportesSemana, responsableSemana, ahorrosParciales] = await Promise.all([
      ...
      prisma.socio.findMany({
        ...
        select: {
          id: true, nombres: true, apellidos: true, numeroCuenta: true, saldoAhorros: true,
          ahorros: {
            where: { rondaId },
            select: { semana: true, monto: true },
          },
        },
    ...
    return NextResponse.json({
      ...
      totalAhorrosSemana,
      items,
      sociosParciales: ahorrosParciales.map(s => {
        const totalAcumulado = s.ahorros.reduce((sum, a) => sum + Number(a.monto), 0);
        const ahorroEstaSemanaMov = s.ahorros.find(a => a.semana === semanaNum);
        return {
          socioId: s.id,
          socio: { nombres: s.nombres, apellidos: s.apellidos, numeroCuenta: s.numeroCuenta, saldoAhorros: Number(s.saldoAhorros) },
          ahorroSemanaActual: Number(ahorroEstaSemanaMov?.monto ?? 0),
          ahorroRegistradoSemana: !!ahorroEstaSemanaMov,
          totalAcumulado,
          objetivo: Number(ronda.ahorroObjetivoPorSocio ?? 0),
          ahorroRestante: Math.max(Number(ronda.ahorroObjetivoPorSocio ?? 0) - totalAcumulado, 0),
          historial: s.ahorros.sort((a, b) => a.semana - b.semana),
        };
      }),
    });
  } catch (err: any) {
