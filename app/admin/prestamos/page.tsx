...
export default function AdminPrestamosPage() {
  ...
  async function buscar() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/prestamos/activos?rondaId=${rondaId}`);
      const d = await res.json();
      setPrestamos(d.prestamos ?? d ?? []);
  ...
  async function guardar() {
    ...
    const advertencia = recalcula ? "\n\n⚡ ATENCIÓN: Cambiar monto, tasa o plazo recalculará la tabla de amortización (cuotas no pagadas)." : "";
    if (!confirm(`¿Confirmar cambios en préstamo de ${editando.socio?.nombres}?${advertencia}\n\nQuedará en bitácora.`)) return;
    setSaving(true);
    try {
  ...
  async function cambiarEstado(id: number, estado: string, socio: string) {
    if (!confirm(`¿Cambiar estado del préstamo de ${socio} a "${estado}"?\n\nQuedará en bitácora.`)) return;
    try {
    ...
    <div className="space-y-4 p-3 sm:p-6">
      ...
      {msg && (
        <div className={cn("rounded-xl p-3 text-sm", msg.ok ? "bg-emerald-50 border border-emerald-200 text-emerald-700" : "bg-red-50 border border-red-200 text-red-700")}>
          <p>{msg.text}</p>
          {msg.efectos?.map((e: any, i: number) => <p key={i} className="text-xs mt-1">↳ {e.descripcion}</p>)}
        </div>
      )}
                ...
                {prestamos.map((p: any) => (
                  <>
                    <tr key={p.id} className="border-t hover:bg-gray-50/60">
                      ...
                      <td className="px-4 py-3 text-center">{p.plazoMeses} meses</td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex gap-1.5 justify-center">
                          ...
                          {p.estado === "ACTIVO" && (
                            <button onClick={() => cambiarEstado(p.id, "CANCELADO", `${p.socio?.nombres}`)}
                              className="rounded-lg border border-gray-200 px-2.5 py-1 text-xs text-gray-600 hover:bg-gray-100">Cancelar</button>
                          )}
                        </div>
                      </td>
                    </tr>
